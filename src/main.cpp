...
String phoneNumber;  // Номер для добавления/удаления в белый список
...
void extractPhoneNumber(const String &sms)
{
  ...
  if (endIndex > plusIndex)
    phoneNumber = sms.substring(plusIndex, endIndex);
  else
    phoneNumber = sms.substring(plusIndex);
...
void handleAddNumberCommand(const String &sms)
{
  ...
  if (addWhitelistNumber(phoneNumber))
  {
    sendSMS(currentSender, "Whitelist:\nAdded " + phoneNumber);
  }
  else
...
void handleDeleteNumberCommand(const String &sms)
{
  ...
  if (deleteWhitelistNumber(phoneNumber))
  {
    sendSMS(currentSender, "Whitelist:\n Deleted " + phoneNumber);
  }
  else
